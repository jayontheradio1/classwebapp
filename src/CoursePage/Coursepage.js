...
const Coursepage = (props) => {
  console.log(props)
  const { courseName1, professorName1 } = useParams();
  const name = decodeURIComponent(courseName1);
  const profname = decodeURIComponent(professorName1);
  const [professorName, setProfessorName] = useState(name);
  const [courseName, setCourseName] = useState(profname);
